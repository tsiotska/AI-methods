class NGramStates {
  constructor(sourceText, order, options) {
    const wordsArray = sourceText.split(/\s+/g).filter(options.filterFunction); // split on words by space symbol
...
const getFollowingWords = (wordsArray, nGram, start = 0) => {
...
const indexOfNgram = (wordsArray, nGram, start = 0) => {
